...
- flash_l4t_nvme_rootfs_ab.xml is an example partition configuration used for
rootfs ab feature on external storage.

Steps:
- Run this command from the Linux_for_Tegra folder:
$ sudo ./tools/kernel_flash/l4t_initrd_flash.sh --external-device <external-device> \
...
- Use --external-only to flash only the external storage device.
  Without providing option "--external-only" to above command line, the command will
  flash both internal and external storage devices.
...
$ sudo BOARDID=<BOARDID> FAB=<FAB> BOARDSKU=<BOARDSKU> BOARDREV=<BOARDREV> 
./tools/kernel_flash/l4t_initrd_flash.sh --no-flash --massflash 5 jetson-xavier-nx-devkit-emmc mmcblk0p1
...
First step: Put the device into recovery mode, then generate encrypted rootfs
...
First step: Put the device into recovery mode, then generate an encrypted root
filesystem for the internal device:
$ sudo ROOTFS_ENC=1 ./tools/kernel_flash/l4t_initrd_flash.sh --no-flash jetson-xavier internal
...
The command is as following if a Jetson in recovery mode is connected:
