...
def __check_i2c_fw_rules_node(node):
...
def __sign(header, data):
    signed_len = _align(len(data) + __signature_len, __dtb_alignment)

    header.pad(signed_len - len(data))

    header_data = header.pack()
    dtb_data = data[_dtb_header_len:]
    padding = b'\x00' * (signed_len - len(data) - __signature_len)
    ...
    signed_data = header_data + dtb_data + padding + signature_data

    header.check(signed_data)

    return signed_data

def __verify(signed_data, unsigned_data):
    assert len(signed_data) % __dtb_alignment == 0

    diff_len = len(signed_data) - len(unsigned_data)
    assert diff_len >= __signature_len
    assert diff_len < __signature_len + __dtb_alignment

    # Verify DTBs are the same except for totalsize, padding and signature
    ...
    dtb_data = signed_data[_dtb_header_len:-__signature_len]
...
def __main():
            ...
            FdtRule('i2c-busses', __check_i2c_busses, subrules=[
                FdtRule('.+', __check_i2c_busses_node, subrules=[
                    FdtRule('i2c_firewall_rules', __check_i2c_fw_rules, subrules=[
                        FdtRule('.+', __check_i2c_fw_rules_node),
                    ]),
                ]),
            ]),
    ...
    signed_data = __sign(header, data)
    __verify(signed_data, data)
