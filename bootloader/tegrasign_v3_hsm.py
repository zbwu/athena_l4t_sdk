...
'''
Checks to see if the HSM aes key is a string of zeros or not
'''
def is_zero_aes_hsm(p_key):
...
'''
@brief The routine that invokes aes-cmac on the buffer
@Note: This routine is expected to be replaced by OEM's own HSM implementation

@param[in] buf Buffer to be operated on
@param[in] p_key SignKey class which has info needed: filename, mode
@note The SignKey class mode define determines bit size operation
...
def do_aes_cmac_hsm(buf, p_key):
    ...
    if p_key.hsm.is_fskp_mode() and (p_key.filename != 'None'):
        cipher = 'aes-256-cbc'
    else:
        cipher = 'aes-128-cbc'
...
@param[in] use_der_key Boolean flag indicating if reading the key from file or not.
           The latter is used when doing encryption followed by derivation, so file read is skipped
...
#openssl dgst -sha256 -mac hmac -macopt hexkey:11111111111111111111111111111111 -out hmac.txt rcm_0.rcm

#openssl dgst -mac cmac -macopt cipher:aes-128-cbc -macopt hexkey:0123..F -binary -out aes_cmac.dgst file.input
...
def do_aes_gcm_hsm(buf, p_key, use_der_key = False):
    ...
    # to write to file
    # order: sizes then data for: key, length, buf, buff_dest, result_name, iv, aad, tag, verify, file
...
'''
@brief The routine that invokes aes-cbc on the buffer
@Note: This routine is expected to be replaced by OEM's own HSM implementation

@param[in] buf Buffer to be operated on
@param[in] p_key SignKey class which has info needed: filename, mode, iv, hsm type
...
def do_aes_cbc_hsm(buf, p_key):
    ...
    runcmd = "openssl enc -e -%s -nopad -in %s -out %s -K %s -iv %s" % (cipher, tmpf_in, tmpf_out, key, iv)
    info_print('[HSM] calling %s' % runcmd)
    try:
        subprocess.check_call(runcmd, shell=True)
    except subprocess.CalledProcessError:
    ...
    info_print('[HSM] ' + cipher + ' is done... return')
...
@retval buffer do_hmac_sha256_hsm() returns the operated buffer
'''
...
@retval buffer do_aes_gcm_hsm() returns the encrypted buffer
'''
...
@param[in] params_slist List array of 8 items to be used:
              DecKdkKdkKdd: product of counter mode(dec kdk + dec kdkd), to be ran with hmachsha for KDF
              DecKdkIv : iv for dec kdk
              DecKdkAad: aad for dec kdk
              Plaintext: plaintext for sbk
              DecKdkMsg: product of counter mode(dec kdk label), to be ran with hmachsha for KDF
              BlKdkMsg : product of counter mode(bl kdk label), to be ran with hmachsha for KDF
              KdkMsg   : product of counter mode(kdk label), to be ran with hmachsha for KDF
              DkMsg    : product of counter mode(dk label), to be ran with hmachsha for KDF

@param[out] kdf_list List array of 5 items to be operated on:
              ...
              flag: flag to indicate if ODM enable or ODM disable
              src buffer[out]: buffer to be encrypted with DK key
              Note: The tag will be saved in kdf_list[KdfArg.TAG]
              Note: The encrypted buffer will be saved in kdf_list[KdfArg.SRC]

@retval True for success, False otherwise
'''
def do_kdf_oem_hsm(params_slist, kdf_list):
    ...
    KdkMsg = str_to_hex(params_slist[6])
    DkMsg = str_to_hex(params_slist[7])
    use_derivation_key = True
    ...
    p_key.kdf.iv.set_buf(DecKdkIv)
    p_key.kdf.aad.set_buf(DecKdkAad)
    p_key.len = len(p_key.key.aeskey)

    # If odm-enc is on, use plaintext as sbk, else derive dec_kdk as following:
    #    1) dec_kdk input = hash = kdf(kdk+kdd, deckdk_msg)
    #    2) dec_kdk key = aes-gcm, stored as DecKdkKey

    if kdf_list[KdfArg.FLAG] == DerKey.OEM_NO_ODM:
    ...
    info_print('[HSM] KDK is ' + hex_to_str(p_key.key.aeskey))

    p_key.key.aeskey = do_hmac_sha256_hsm(DkMsg, p_key, use_derivation_key)
    info_print('[HSM] DK is ' + hex_to_str(p_key.key.aeskey))

    p_key.len = len(kdf_list[KdfArg.SRC])
    ...
    kdf_list[KdfArg.SRC] = do_aes_gcm_hsm(kdf_list[KdfArg.SRC], p_key, use_derivation_key)
    kdf_list[KdfArg.TAG] = p_key.kdf.tag.get_hexbuf()

    return True

'''
@brief The routine that performs key derivation
@Note: This routine is expected to be replaced by OEM's own HSM implementation
...
@retval True for success, False otherwise
'''
def do_derive_cbc_hsm(buf, p_key):
    # Note some kdf is using string operation
    is_hex = False
    L = 128 # key length in bits

    p_key.kdf.get_composed_msg(L, is_hex)
    ...
    if (p_key.hsm.get_type() not in [KeyType.SBK, KeyType.KEK0]):
        raise tegrasign_exception("[HSM] ERROR: type not supported: " + key_type)

    result_name = os.path.splitext(filename)[0] + '_encrypt' + os.path.splitext(filename)[1]

    with open_file(tmpf_in, 'wb') as f:
        write_file(f, p_key.kdf.get_hexmsg())

    info_print('[HSM] do_aes_cmac_hsm: read an AES key filename=%s. Key type is %s' % (key_file, key_type))

    key = get_sbk_key_content(p_key)

    # do aes-cmac on msg to derive the key
    cipher = 'aes-128-cbc'
    ...
    try:
        ...
        raise tegrasign_exception("[HSM] ERROR: failure in running %s Error %s" % (runcmd, str(e)))
    finally:
        os.remove(tmpf_in)

    with open_file(tmpf_out, 'rb') as f:
        der_key = hex_to_str(f.read())
        info_print('[HSM] Derived key is ' + der_key)

    os.remove(tmpf_out)

    info_print('[HSM] aes cmac is done...')
    ...
    runcmd = "openssl enc -e -%s -nopad -in %s -out %s -K %s -iv %s" % (cipher, tmpf_in, result_name, der_key, p_key.kdf.iv.get_strbuf())
    info_print('[HSM] calling %s' % runcmd)
    try:
        subprocess.check_call(runcmd, shell=True)
    except subprocess.CalledProcessError as e:
    ...
    if check_file(result_name):
        info_print('[HSM] aes-cbc-128 is done... return')
        return True
    info_print('[HSM] aes-cbc-128 is done but result file is not found. Please check')
    return False
