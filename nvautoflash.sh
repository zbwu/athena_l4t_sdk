...
read_eeprom ()
{
	...
	if [ "${tegraid}" = "0x23" ]; then
		# TODO: - nvbugs/3458086 : For 0x23, generic dump eeprom function is disabled.
		# Therefore, we need to use agx orin specific function to retrieve eeprom as a WAR
		# When we get a board agnostic way to retrieve cvm.bin, we can replace this function
		popd > /dev/null 2>&1 || exit 2;
		pushd "${curdir}" > /dev/null 2>&1 || exit 2;
		cmd="NO_ROOTFS=1 \"${curdir}/flash.sh\" --no-flash jetson-agx-orin-devkit mmcblk0p1"
	fi
	echo "${cmd}";
	eval "${cmd}";
...
if [ "${target_board}" = "jetson-agx-xavier-industrial" ]; then
	echo "Error: Target board ${target_board} is not supported for this release." >&2;
	exit 15;
fi
